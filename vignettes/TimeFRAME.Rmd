...
```{r}
system.time({ fit.gp <- fit_gp(model, meas, t = t, eta = eta, iter = 2000) })
```
...
```{r}
system.time({ fit.dgp <- fit_dgp(model, meas, t = t, eta = eta, iter = 2000) })
```
...
```{r}
system.time({ fit.hdgp <- fit_dgp(model, meas, t = t, eta = eta, estim.rho = TRUE, iter = 2000) })
```
