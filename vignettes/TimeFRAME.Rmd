...
```{r}
as.data.frame(fit.frame) %>%
  inner_join(d.weights %>%
               pivot_longer(-t, names_to = "variable"),
             by = c("t", "source" = "variable")) %>%
  ggplot() +
  geom_ribbon(aes(x = t, ymin = `2.5%`, ymax = `97.5%`, fill = source), alpha = 0.4) +
  geom_line(aes(x = t, y = value, group = source)) +
  geom_line(aes(x = t, y = mean, col = source), linewidth = 1) +
  ylim(0, 1) +
  facet_grid(source ~ .) +
  xlab("Time") + ylab("Source Contribution and Fractionation Weight") + 
  labs(col = "Source", title = "Independent FRAME Model") +
  guides(fill = "none") +
  theme_bw()
```
...
as.data.frame(fit.glm) %>%
  ...
  facet_grid(source ~ .) +
  xlab("Time") + ylab("Source Contribution and Fractionation Weight") + 
  labs(col = "Source", title = "Spline GLM Model") +
  guides(fill = "none") +
  theme_bw()
```
...
as.data.frame(fit.gp) %>%
  ...
  facet_grid(source ~ .) +
  xlab("Time") + ylab("Source Contribution and Fractionation Weight") + 
  labs(col = "Source", title = "GP Model") +
  guides(fill = "none") +
  theme_bw()
```
...
as.data.frame(fit.dgp) %>%
  ...
  facet_grid(source ~ .) +
  xlab("Time") + ylab("Source Contribution and Fractionation Weight") + 
  labs(col = "Source", title = "DGP Model") +
  guides(fill = "none") +
  theme_bw()
```
