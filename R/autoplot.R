...
autoplot.FrameFit <- function(object, ...) {
  ...
  as.data.frame(object) %>%
    ...
    ggplot2::labs(col = "Source", title = sprintf("Mean and 95%% Credible Interval (%s)", object$model_name)) +
...
autoplot.FrameFit.stationary <- function(object, ...) {
  as.data.frame(object) %>%
    ...
    ggplot2::ylim(0, 1) +
    ggplot2::xlab("Source") + ggplot2::ylab("Source Contribution and Fractionation Weight") +
    ggplot2::labs(col = "Source", title = sprintf("Median, 50%% and 95%% Credible Intervals (%s)", object$model_name)) +
