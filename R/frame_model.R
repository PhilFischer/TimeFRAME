...
#' @param sources data frame with \eqn{d} columns corresponding to isotopic measurement dimensions and \eqn{2K} rows,
#' the first \eqn{K} corresponding to source locations and the next \eqn{K} corresponding to source spreads.
...
frame_model <- function(sources, frac = data.frame()) {
  ...
  model <- list(sources = as.data.frame(sources), frac = as.data.frame(frac), d = d, K = K, L = L)
...
print.FrameModel <- function(x, ...) {
  ...
  if(x$d > x$K+x$L-1) cat(sprintf("(%s extra dofs)", x$d - x$K - x$L + 1))
  cat("\n")
}
