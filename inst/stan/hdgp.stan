...
transformed parameters {
  ...
  matrix[N, N] LL_f;
  matrix[N, N] G_f = gp_exp_quad_cov(t, 1.0, corl);
  matrix[N, N] LL_r;
  matrix[N, N] G_r = gp_exp_quad_cov(t, 1.0, corl_r);
...
model {
  target += inv_gamma_lpdf(corl | 5, rho * 5);
  target += inv_gamma_lpdf(corl_r | 5, rho_r * 5);
